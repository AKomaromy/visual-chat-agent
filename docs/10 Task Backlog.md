...
**Status: ✅ PASSED LIVE (2026-07-20), deployed to Trigger.dev Cloud (`prod`, version `20260720.2`).** `chatId` keying comes from `useChat({ id: chatId })` in `app/components/chat.tsx` (`${profileId}-chat`), not a separate idempotency key yet. Artifact persistence (writing to the `artifacts` table) is still not implemented — not part of Task 2's or Task 5's stated acceptance criteria, tracked as open follow-up in `docs/14 Engineering Handoff.md`. Model provider is OpenAI (`gpt-4o`) after the Anthropic org behind this project's original key was disabled — `docs/11 Risks.md` R-22. Two real defects were found and fixed before this passed clean: `clientData` wasn't wired to the system prompt (fixed via `onChatStart` + `chat.prompt.set()`), and a ClickHouse column-shadowing bug in the Map query (fixed in `lib/briefing.ts`, caught via live browser verification). The OpenAI `429 insufficient_quota` blocker has cleared — verified live twice, both profiles, zero errors, manifests differ, and once more through the actual browser UI end to end.
