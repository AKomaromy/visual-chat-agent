...
**Status: 🟡 implemented at `trigger/chat.ts`, deployed live to Trigger.dev Cloud (`prod`, version `20260719.3`), live-tested and blocked on OpenAI billing (external, not this project).** `chatId` keying comes from `useChat({ id: chatId })` in `app/components/chat.tsx` (`${profileId}-chat`), not a separate idempotency key yet. Artifact persistence (writing to the `artifacts` table) is not implemented — that lands with Task 5. Model provider swapped from Anthropic to OpenAI (`gpt-4o`) after the Anthropic org behind this project's key was disabled — see `docs/11 Risks.md` R-22. A real defect (the model never learned which profile was active — `clientData` wasn't wired to the system prompt) was found and fixed via `onChatStart` + `chat.prompt.set()` before the first live attempt. `OPENAI_API_KEY` and `CLICKHOUSE_*` are both confirmed correctly set in the Cloud `prod` environment (the run reached `WAITING` with the right session/payload). The live call itself fails with OpenAI `429 insufficient_quota`, confirmed twice including after the user's reported billing fix — needs resolution on platform.openai.com, not in this codebase.
