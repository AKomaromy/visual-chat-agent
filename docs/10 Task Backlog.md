...
**Status: ✅ PASSED LIVE (2026-07-20), deployed to Trigger.dev Cloud (`prod`, version `20260720.3`).** `chatId` keying comes from `useChat({ id: chatId })` in `app/components/chat.tsx`, minted per-profile-per-browser-tab via `getOrCreateChatId()` (a fresh `${profileId}-chat-<uuid>`, persisted in `sessionStorage`) rather than a fixed constant — changed during Session 6 after a fixed `${profileId}-chat` id was found to permanently lock out a profile once its Trigger.dev Session closed (`docs/11 Risks.md` R-38). Artifact persistence (writing to the `artifacts` table) is still not implemented — not part of Task 2's or Task 5's stated acceptance criteria, tracked as open follow-up in `docs/14 Engineering Handoff.md`. Model provider is OpenAI (`gpt-4o`) after the Anthropic org behind this project's original key was disabled — `docs/11 Risks.md` R-22. Real defects were found and fixed before this passed clean, all via live verification rather than static checks: `clientData` wasn't wired to the system prompt (fixed via `onChatStart` + `chat.prompt.set()`); a ClickHouse column-shadowing bug in the Map query; a permanently-dead fixed `chatId` (R-38); and the model's raw text rendering as an extra, uncontracted view alongside the workspace (R-39). The OpenAI `429 insufficient_quota` blocker has cleared — verified live multiple times, both profiles, zero errors, manifests differ, through both the session-protocol script and the actual browser UI end to end.

### WF-B — `seed-gdelt`
**Status: 🟡 implemented at `trigger/seed-gdelt.ts`, deployed live to Trigger.dev Cloud (`prod`, version `20260719.3`), live-tested and blocked on a real GDELT outage (external, not this project).** `CLICKHOUSE_*` credentials in `prod` are confirmed correct — the task reached the ClickHouse no-op check before failing on the GDELT fetch. Two consecutive live attempts both failed with a genuine `ConnectTimeoutError` reaching `api.gdeltproject.org`, confirmed as a real outage (not environment-specific) via an independent `curl` failure at the same time while general internet and GDELT's own marketing site responded normally — see `docs/11 Risks.md` R-05. Re-run once GDELT is reachable again.
...
**Status: 🟢 schema implemented and in real use at `lib/visual-response.ts` (`visualResponseManifestSchema`); persistence to `artifacts` not yet wired.** The envelope matches this spec; three additive fields were added during the visual-workspace build so every displayed mark traces to real evidence: `timeline[].evidenceIds` and `map[].evidenceIds` (which of the manifest's evidence entries fall in that bucket/cell — computed server-side in `lib/briefing.ts` from already-fetched rows, not a new query), `map[].lat`/`lon` (decoded via ClickHouse's `h3ToGeo`, so the frontend never computes geography itself), and `evidence[].location`/`relevanceContext` (for the Evidence Drawer). None of this changes the envelope shape or view order. The "every item has a resolvable evidence article ID" check and persistence to `artifacts` are still open — the frontend validates the manifest against this schema client-side (`app/components/chat.tsx`) before rendering, which catches a malformed manifest, but nothing yet re-validates evidence-ID resolvability server-side beyond what `lib/briefing.ts` already guarantees by construction.
...
1. ✅ `mirror-agent` skeleton with a fixture manifest (Task 2) — passed live 2026-07-20, now with the real `getBriefing` tool, not the fixture.
...
6. ✅ Impact Radar, Timeline, Map renderers + Evidence Drawer (`app/components/workspace/*.tsx`) — the drawer renders from the stored title/source/date/location, with the outbound article URL as a secondary "View source" link, so one dead external link during the demo can't break the "every claim has evidence" requirement (`12 Scope Gate.md` §7.4). Map uses `world-atlas`/`d3-geo`/`topojson-client` — bundled offline topology data, no live map-tile dependency, same reasoning as the Evidence Drawer's own no-live-fetch rule. Timeline is hand-rolled CSS, not a charting library.
7. ✅ Coordinated selection/filtering across the three renderers — one shared `WorkspaceSelection` (`app/components/workspace/types.ts`) drives highlighting across Impact Radar, Timeline, and Map and determines what the Evidence Drawer shows. Verified live: selecting a radar signal, timeline bar, or map point all correctly highlight the others and surface matching evidence.
