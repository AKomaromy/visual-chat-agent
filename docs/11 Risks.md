...
**Final bounded check, same Session 7 hardening pass (post-credential-rotation):** before any further ingestion attempt, made exactly one minimal GDELT DOC 2.0 request (the same endpoint/essential params `seed-gdelt.ts` uses: `mode=ArtList`, `maxrecords=1`, `format=json`) to establish current reachability without committing to a full ingestion run. Result: `HTTP 429`, ~9.8s elapsed, no `Retry-After` header. Per the bounded protocol for this check (respect `Retry-After` when given, otherwise make at most one delayed retry using the codebase's own established 5s backoff step, no concurrency), one delayed retry was made after a 5s wait — it did not return a 429 or any response at all, timing out at 10s with no data. Two consecutive failures (rate-limited, then unreachable) on the very first, minimal, no-side-effects request meant the bar for proceeding to the full sequential ingestion (§4 of that check's own protocol) was not met, so the attempt stopped there — no topic queries were fetched, nothing was written to ClickHouse, and fixtures were left completely untouched. This is consistent with R-33/R-05's established pattern (intermittent, not a hard block) rather than a new failure mode. No further GDELT attempt was made this pass, per the "stop pursuing GDELT for the current submission" instruction once the bounded check itself didn't clear.

**Root-cause refinement, fresh session 2026-07-22 (no live credentials available this session — see docs/14 Engineering Handoff.md §7 note below):** external research (GDELT's own "Behind The Scenes: API Quotas" blog post, plus a corroborating `gdelt-doc-api` GitHub issue) surfaced a specific, documented mechanism that matches every observation recorded above: GDELT enforces a strict per-IP request rate, and once tripped, the resulting block is IP-level and **lasts significantly longer than the violation that caused it** — and this disproportionately hits shared/multi-tenant cloud platforms, because unrelated tenants' traffic on the same egress IP pool counts against the same limit. Trigger.dev Cloud is exactly this kind of shared platform, which would explain why a local `curl` and Trigger.dev Cloud's own egress have repeatedly disagreed about reachability at the same moment (R-05's 2026-07-20 addendum above) — they're different IP pools with independent standing, not evidence of a flaky/intermittent GDELT. Separately, the linked GitHub issue reports GDELT silently rate-limiting requests with a non-browser-shaped `User-Agent`; this project's `seed-gdelt.ts` was sending `"mirror-hackathon-seed/1.0"`, a string that reads as a bot signature. **Action taken:** `trigger/seed-gdelt.ts` was updated to send a browser-shaped `User-Agent` with contact info, and its 429 backoff was changed from 5s/10s/15s/20s to GDELT's documented 60s/120s guidance (`maxDuration` raised to 600s accordingly). This is a plausible, evidence-backed fix, **not yet live-verified** — this session started in a fresh container with no `.env.local` and no Trigger.dev CLI login, so the fix could be written and statically checked (`typecheck`/`lint` clean) but not deployed or run against `prod`. A secondary attempt to get independent reachability signal via `WebFetch` (a different network path entirely) was inconclusive: it 403'd on `gdeltproject.org`, but it *also* 403'd on a neutral control (`example.com`) in the same session, indicating that path had its own problem rather than telling us anything new about GDELT. **Next step once credentials are available:** redeploy (commit `bc79a7c`) and run `scripts/verify-task4-seed-gdelt-live.ts` against `prod`. If the UA/backoff fix alone doesn't clear it, the shared-IP theory points toward Google BigQuery's public GDELT mirror (`gdelt-bq.gdeltv2.*`) as the most robust next option, since it's a different provider/network path entirely — but that requires new GCP credentials and is a real infrastructure change needing explicit sign-off, not a same-session pivot.
...
### R-45 — `mirror-agent`'s default 60s `maxDuration` was too tight, causing a real `prod` timeout on a cold-start run

**Category:** Technical/Trigger.dev configuration. **Likelihood:** confirmed once (a genuine `MAX_DURATION_EXCEEDED` against `prod`); several near-miss local runs the same session (50-59s) show it wasn't a one-off fluke. **Impact:** would have intermittently broken the demo/judging path with no code or credential problem to blame — exactly the kind of failure that looks like a regression but isn't one.

`mirror-agent` (`trigger/chat.ts`) had no per-task `maxDuration` override, so it inherited `trigger.config.ts`'s project-wide 60s default. Discovered while re-verifying `prod` after Andrew rotated its credentials (a routine reverification, not a new feature): the first post-redeploy run hit `MAX_DURATION_EXCEEDED`. Isolated as a genuine duration-margin issue, not a credential regression, in two steps: (1) an immediate retry against the exact same rotated credentials passed cleanly, ruling out an auth problem; (2) several *local* runs earlier in the same session had already completed in the 50-59s range against this same 60s budget, well before any credential change — meaning the margin was already this tight beforehand, and a slower cold-start machine spinning up a fresh worker for the just-redeployed version was enough to tip one run over 60s. Same underlying pattern as R-37 (`seed-gdelt`'s duration budget not accounting for its own backoff logic), just surfacing on the chat agent instead of the ingestion task.

**Mitigation:** added a per-task override, `maxDuration: 180`, directly on the `chat.agent({...})` call in `trigger/chat.ts` (same mechanism `chat.agent()` exposes for all standard `TaskOptions`, per the bundled SDK reference docs) — no change to agent behavior, model, prompt, or tool logic. Redeployed as `prod` version `20260721.2`. Re-verified live against `prod`: the locked default question for both Profile A and Profile B, and the supported non-default question, all completed without any duration error.

---

## 11. Source notes

Synthesized from the Deep Research corpus's `risk-register.md` (canonical 30-risk register) merged with `roadmap.md` §6 (which covered the same risk surface from a product/roadmap angle with materially the same items — no new risk IDs were created from it, only elaboration). R-05, R-08, and R-18 were rewritten for GDELT-primary ingestion per `05 Architecture.md` ADR-008/009; likelihood ratings for R-05 and R-18 were raised relative to the source register to reflect the single-source P0 dependency this creates, and that trade-off is recorded explicitly in §1 and §8. R-31 and R-32 added during Stage A implementation; R-33 and R-34 added during the Trigger.dev Cloud deployment session; R-35 and R-36 added during the visual-workspace build session, both caught by this project's practice of live-verifying every query and every rendered view rather than trusting static checks alone; R-37, R-38, and R-39 added during the Session-6 submission-readiness pass — R-37 when a GDELT reachability retry surfaced an in-repo `maxDuration` defect; R-38 and R-39 when verifying the actual deployed demo flow for both profiles through the real browser UI surfaced a session-lockout bug and a Demo Contract view-order violation, both fixed before they could reach a recording; R-40 through R-44 added during the Session-7 hostile-judge hardening pass — R-40 and R-41 from an independent code-level review of the already-shipped visual workspace (an evidence/count consistency gap and a mislabeled recency-as-momentum indicator); R-42 and R-43 from making `chat.agent()`'s tool call materially depend on the user's actual question, which in turn surfaced a real gpt-4o tool-calling reliability defect (fabricating values for optional parameters) not caught until live testing; R-44 from discovering the live-verification script used to confirm all of the above had its own false-positive bug; R-45 added when a routine post-credential-rotation `prod` reverification (same hostile-judge pass) surfaced a genuine `maxDuration` timeout on `mirror-agent`, the same class of defect as R-37 but on the chat agent rather than the ingestion task (see `docs/14 Engineering Handoff.md` for current build status).
