...
**Update, fresh session 2026-07-22:** external research pinned down a specific, documented cause matching the observed 429-then-timeout pattern — GDELT's per-IP throttling produces a block that outlasts the triggering request and disproportionately hits shared cloud egress pools like Trigger.dev Cloud's (full detail: `docs/11 Risks.md` R-05's newest addendum). A low-risk fix (realistic `User-Agent`, GDELT-documented 60s/120s backoff) was written, statically verified, committed, and pushed (`bc79a7c`) — **but this session had no `.env.local` and no Trigger.dev CLI login** (fresh ephemeral container), so it could not be deployed or live-tested. **Action needed before Task 4 can move further:** either (a) configure this Claude Code environment with the project's credentials (env vars / setup script) so a future session can deploy + run `scripts/verify-task4-seed-gdelt-live.ts` unattended, or (b) run that deploy+verify step yourself and report the result. If the UA/backoff fix doesn't clear it, the shared-IP theory points to Google BigQuery's public GDELT mirror as the next, more robust option — but that's a real infra change (new GCP credentials, ingestion rewrite) needing explicit sign-off first, not something to pivot to unprompted.
