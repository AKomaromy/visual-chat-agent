...
**Stage A, Session 1–2, in progress.** Task 1 and Task 3 are fully verified live. Task 2 and Task 4's code are both complete, deployed to Trigger.dev Cloud (`prod`, version `20260720.1`), and **all required credentials are confirmed correctly configured there**. Both tasks were live-tested and both hit a **real external blocker with nothing left to fix in code**: Task 2 on OpenAI account quota (`docs/11 Risks.md` R-22), Task 4 on a live GDELT API outage (`docs/11 Risks.md` R-05). A genuine implementation defect was also found and fixed along the way (§3, Task 2 — `clientData`/`profileId` was never wired to the model).

**Task 5 (real ClickHouse-backed `getBriefing`) is now code-complete and verified against controlled development fixtures** (`lib/briefing.ts`, `trigger/load-dev-fixtures.ts`, `trigger/seed-profile-cards.ts`) — done specifically to make forward progress while Tasks 2 and 4 wait on external conditions, per explicit instruction. This is **not** live-data-verified — that still needs Task 4's real GDELT seed. Two permanent, committed rerun scripts (`scripts/verify-task2-chat-live.ts`, `scripts/verify-task4-seed-gdelt-live.ts`) are ready to re-run Task 2/Task 4's live acceptance tests the moment each external condition clears. The Session 3–6 work (renderers, personalization, demo) has not started.
...
Task 2 and Task 4's code are **done, deployed, and live-tested** — dispatch, credentials, and (for Task 2) a real bug fix are all confirmed working, blocked only by two external conditions (§7). Task 5's code is **done and fixture-verified** — the query logic is proven correct; it only needs real data to be verified end-to-end. Two permanent rerun scripts are ready in `scripts/`:

1. **Once OpenAI quota is fixed:** `set -a; source .env.local; set +a && npx tsx scripts/verify-task2-chat-live.ts`. Runs both profiles through the real session protocol against `prod`, prints verdicts/top-radar-items, and exits non-zero with a clear reason if it's still blocked or if the manifests don't differ.
2. **Once GDELT is reachable again** (sanity-check first: `curl "https://api.gdeltproject.org/api/v2/doc/doc?query=test&mode=ArtList&maxrecords=1&format=json"`): `set -a; source .env.local; set +a && npx tsx scripts/verify-task4-seed-gdelt-live.ts`. Triggers `seed-gdelt` against `prod`, waits on `finishedAt` (not status strings — see §8), and checks the result against `docs/13 Demo Contract.md` §4's thresholds automatically.
3. After Task 4's real seed lands: **clear the dev fixtures** (`tasks.trigger("load-dev-fixtures", { clear: true })`) so `articles` holds only real rows, then re-run the same spot-check §3's Task 5 section describes (`getBriefingManifest("profile-a")` / `"profile-b"`, confirm differentiation) against the real data — the fixture-based result already proved the query logic works; this step just confirms it holds against real GDELT titles too. Do the manual diversity check by hand (`docs/12 Scope Gate.md` §7.2) at the same time.
4. Once both Task 2 and Task 4 pass live, update `docs/09 Sprint Plan.md`'s status lines to ✅, commit, and move to Session 4 (Impact Radar/Timeline/Map renderers + Evidence Drawer — `docs/10 Task Backlog.md` §5 build order items 6-7).
...
2. Quickly re-check both external blockers:
   - OpenAI: `curl https://api.openai.com/v1/chat/completions` with `$OPENAI_API_KEY` from `.env.local` — if it no longer returns `429 insufficient_quota`, run `scripts/verify-task2-chat-live.ts` (§9).
...
3. **If either is now clear:** run that script — no new code, no new credentials, no redeploy needed. Update `docs/09`/`10` status once it passes.
4. **If both are still blocked:** there is no more Stage A code work to pull forward — Task 5 is already done (fixture-verified) and the renderer work (Session 4) isn't scoped yet. Report status and wait; don't manufacture busywork.
5. Use `npx trigger.dev@4.5.4 <command>`, not `@latest` — see §4/§8 for why.
6. Do not re-run Task 1's connectivity check, Task 3's `init-schema`, or Task 5's fixture-verification — all already passed. Do not re-attempt Task 2 or Task 4 through the **local** `trigger.dev dev` worker for GDELT/OpenAI calls specifically — that path is root-caused as broken in this sandbox type (R-33) for *new third-party hosts*; the Cloud deploy is the permanent fix. (The local worker is fine for ClickHouse-only tasks like `seed-profile-cards`/`load-dev-fixtures`, which is how this session ran them.)

---

## 11. Source notes

Written per an explicit handoff request at the end of a Stage A implementation session. Updated across many subsequent sessions/turns (per `CLAUDE.md`'s standing authorization to continue implementation without re-approval, and later per explicit approved sequencing/infra changes and one explicit instruction to proceed to Task 5 despite two open external blockers): after Task 3 was verified live; after Task 4 was written and its local live-run blocker root-caused; after that was resolved by deploying to Trigger.dev Cloud; after a supplied `ANTHROPIC_API_KEY` turned out to belong to a disabled org, prompting a provider swap to OpenAI; after all remaining Cloud-dashboard credential gaps were resolved and live verification was attempted, surfacing and fixing a real `clientData`/`profileId` wiring defect, and confirming Task 2 and Task 4 both dispatch correctly but are blocked by OpenAI account quota and a live GDELT outage respectively; and, in this revision, after Task 5 (the real ClickHouse-backed `getBriefing`) was implemented and verified against controlled, clearly-isolated development fixtures — a deliberate choice to keep making real progress while those two external conditions remain outside this session's control, per explicit instruction, without letting fixture data ever be mistaken for satisfying Task 4's own acceptance bar. Two permanent rerun scripts (`scripts/verify-task2-chat-live.ts`, `scripts/verify-task4-seed-gdelt-live.ts`) are ready for the moment either blocker clears. Reflects repository state as of commit `b984536` plus the Task 5 implementation and documentation updates committed alongside this revision. Does not modify `CLAUDE.md`, `docs/13 Demo Contract.md`, `docs/12 Scope Gate.md`, `docs/08 MVP.md`, `docs/02 Product.md`, or `docs/01 Vision.md` — all five remain frozen, per instruction.
