...
**Session 7 (security rotation + hardening/polish) done, one item pending Andrew.** Executed `docs/19`'s "Instructions for Claude Code": a security audit confirmed no secret has ever been committed to Git (search covered history + working tree for secret-shaped patterns, not just the tracked-file check); **all three of `CLICKHOUSE_PASSWORD`, `TRIGGER_SECRET_KEY`, and `OPENAI_API_KEY` are now confirmed chat-exposed** (the OpenAI one required asking Andrew directly, since this project's own risk log hadn't actually documented it despite an inconsistent summary line asserting it) and need rotation — **pending Andrew's action**, see §7. The runbook's Part D/E (news-provider and LLM-provider abstractions) was explicitly skipped, per Andrew's own decision, as a scope-freeze conflict. Separately, a hardening/polish pass fixed the *root cause* of R-39 (the model was still generating unused narration every turn, not just having it hidden — `trigger/chat.ts`'s system prompt now explicitly forbids it), a stale comment in `lib/briefing.ts` claiming a ClickHouse function did something a JS `URL` call actually does, a permanently-visible debug "Status: ready" line, and an uncentered idle landing view. Added a curated, captioned `demo-assets/` screenshot set. Remaining: Andrew rotates the three credentials (§7), then Part C reverification; recording the actual demo video; Andrew's own review/decision gates.
...
| `OPENAI_API_KEY` | ✅ set | ⚠️ **Confirmed exposed (Session 7 security audit, 2026-07-20).** An earlier version of this row asserted this without R-31's own narrative actually supporting it at the time — flagged, asked, and confirmed directly by the user. All three of `CLICKHOUSE_PASSWORD`, `TRIGGER_SECRET_KEY`, and `OPENAI_API_KEY` require rotation before/around making the repo public — see `docs/19` for the guided procedure. Live-confirmed present and read correctly; the OpenAI account itself has no usable-quota issue anymore (R-22 resolved). |
...
The local-sandbox networking limitation that originally blocked Task 4 (R-33) is resolved via the Cloud deploy. Every credential/dashboard gap from earlier in this session (§5) is resolved. **Task 2's OpenAI quota blocker is resolved** — the account's quota cleared on its own and Task 2 passed live on 2026-07-20 (§3, `docs/11 Risks.md` R-22). Two things are left — one external, one requiring Andrew directly:

**Credential rotation (requires Andrew — not fixable by an agent).** `CLICKHOUSE_PASSWORD`, `TRIGGER_SECRET_KEY`, and `OPENAI_API_KEY` are all confirmed to have been pasted into chat at some point and must be treated as exposed (`docs/11 Risks.md` R-31, updated Session 7). Rotation requires dashboard actions and pasting new values directly into `.env.local`/the Trigger.dev `prod` environment — steps no agent can perform. See `docs/19 Security Rotation and Provider Resilience Runbook.md`'s human checklist for the exact per-credential procedure. Once done, Part C reverification (redeploy, `scripts/verify-task2-chat-live.ts`, a browser pass for both profiles, typecheck/lint/build) should run immediately.
